...
export const metadata = {
  title: 'After Party Scanner — PHAmily Classic',
  robots: { index: false, follow: false },
};

export default function AfterPartyScannerPage() {
  const cookie = cookies().get(SCANNER_SESSION_COOKIE)?.value;
  if (!verifySessionCookie(cookie)) {
    redirect('/scan/login?redirectTo=/scan/after-party');
  }
  return <ScannerPageClient event="after_party" />;
}
